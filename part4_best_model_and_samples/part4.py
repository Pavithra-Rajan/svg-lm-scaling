...
# Sectioned grid: unconditional, temperature sweep, sampling strategies, prefix-conditioned

def render_section(title, subtitle, items):
    """items: list of (label, svg) pairs."""
    cards = []
    for label, svg in items:
        ...
        cards.append(
            ...
            f'<div style="font-size:11px;width:120px;margin-bottom:4px;word-wrap:break-word">{label}</div>'
            f'<div style="width:120px;height:120px;border:1px solid #ccc;overflow:hidden">{clean}</div>'
            f'</div>'
        )
    head = (
        f'<h2 style="font-family:sans-serif;margin-top:24px;margin-bottom:4px">{title}</h2>'
        f'<div style="font-family:sans-serif;font-size:12px;color:#555;margin-bottom:8px">{subtitle}</div>'
    )
    return head + ''.join(cards)

html_sections = []

# 1. Unconditional samples (T=1.0, top_k=50 in this run)
html_sections.append(render_section(
    'Unconditional samples',
    f'{len(unconditional_samples)} samples generated from a bare &lt;svg prefix '
    f'(temperature=1.0, top_k=50).',
    [(f'Unconditional {i+1}', s) for i, s in enumerate(unconditional_samples)]
))

# 2. Temperature comparison (same seed)
html_sections.append(render_section(
    'Temperature sweep (same seed, top_k=50)',
    'Lower T = more conservative; higher T = more diverse but more malformed.',
    [(f'T = {t}', svg) for t, svg in temp_samples.items()]
))

# 3. Sampling strategies
html_sections.append(render_section(
    'Sampling-strategy comparison',
    'Top-k=50 (T=0.8); nucleus / top-p=0.9 (T=0.8); greedy (T=0.01, top_k=1).',
    [
        ('top-k=50',       sampling_results['top_k_50']),
        ('top-p=0.9',      sampling_results['nucleus_p0.9']),
        ('greedy',         sampling_results['greedy']),
    ]
))

# 4. Prefix-conditioned (full SVG = original prefix + model completion)
html_sections.append(render_section(
    'Prefix-conditioned completions',
    f'{len(prefix_results)} partial-SVG prompts; the model is asked to complete each.',
    [(r['description'][:80], r['full_svg']) for r in prefix_results]
))

html_out = (
    '<html><body style="font-family:monospace;padding:16px;max-width:1200px;margin:auto">'
    '<h1 style="font-family:sans-serif">Generated SVG samples &mdash; sp_medium checkpoint</h1>'
    + ''.join(html_sections)
    + '</body></html>'
)
